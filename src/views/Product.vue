<template>
  <div id="product">
    <!-- 이곳을 자유롭게 만들어주세요. -->
  

    <div class="container">
      <!-- Stack the columns on mobile by making one full-width and the other half-width -->
      <div class='row'>
        <div class='col-md-5'>
            <div id="carouselExampleIndicators" class="carousel slide" data-bs-ride="carousel">
            <div class="carousel-indicators">
              <button type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide-to="0" class="active" aria-current="true" aria-label="Slide 1"></button>
              <button type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide-to="1" aria-label="Slide 2"></button>
              <button type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide-to="2" aria-label="Slide 3"></button>
            </div>
            <div class="carousel-inner">
              <div class="carousel-item active">
                <img src="https://contents.sixshop.com/uploadedFiles/48764/product/image_1620794006376.jpg" class="d-block w-100" alt="...">
              </div>
              <div class="carousel-item">
                <img src="https://contents.sixshop.com/uploadedFiles/48764/product/image_1620794006376.jpg" class="d-block w-100" alt="...">
              </div>
              <div class="carousel-item">
                <img src="https://contents.sixshop.com/uploadedFiles/48764/product/image_1620794006376.jpg" class="d-block w-100" alt="...">
              </div>
            </div>
            <button class="carousel-control-prev" type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide="prev">
              <span class="carousel-control-prev-icon" aria-hidden="true"></span>
              <span class="visually-hidden">Previous</span>
            </button>
            <button class="carousel-control-next" type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide="next">
              <span class="carousel-control-next-icon" aria-hidden="true"></span>
              <span class="visually-hidden">Next</span>
            </button>
          </div>
        </div>
        <div class='col-md-2'>
        </div>
        <div class='col-md-5'>
          <div class="card" >
            <div class="card-body">
              <h5 class="card-title" style='text-align: center;'>상품명</h5>
              <h6 class="card-subtitle mb-2 text-muted" style='text-align: right;'>상품 가격</h6>
              <p class="card-text" style='text-align: right;'>배송비</p>
              <div class="btn-group" role="group" aria-label="Button group with nested dropdown" >
                <button type="button" class="btn btn-warning" >수량 선택</button>
                <div class="btn-group" role="group">
                  <button id="btnGroupDrop4" type="button" class="btn btn-warning dropdown-toggle" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false"></button>
                  <div class="dropdown-menu" aria-labelledby="btnGroupDrop4">
                    <a class="dropdown-item" href="#">Dropdown link</a>
                    <a class="dropdown-item" href="#">Dropdown link</a>
                  </div>
                </div>                          
              </div>
              <p class='card-text' style='text-align: right;'>배송 예정일</p>
            </div>
          </div>
          <div class='d-flex justify-content-between align-items-center'>
          <div class = 'col-6 d-grid p-1'>
            <button type="button" class="btn btn-primary">장바구니</button>
          </div>
          <div class = 'col-6 d-grid p-1'>
            <button type="button" class="btn btn-danger">구매하기</button>
          </div>
          </div>
        </div>
      </div>
   
    </div>
    <br>
    <div class='img-container img_detail'>

      <img src="https://t1.daumcdn.net/cfile/tistory/223C843B52C8423631" class="img-fluid" alt="...">
    </div>
      <div class = 'container review_parent'>
        <button type="button" class="btn btn-outline-primary">리뷰 더 보기</button>
        <button type="button" class="btn btn-outline-primary">리뷰 올리기</button>

        <div class = 'container review' id='review_1'>
        <div class='item item1'>★★★★★</div>
        <div class='item item2'>작성자</div>
        <div class='item item3'>작성일자</div>
      </div>
      <div class = 'container review' id='review_2'>
        <div class='item item4'>리뷰 내용...</div>
        <img src="data:image/jpeg;base64," 
        class="img-thumbnail" alt="...">
      </div>
      <div class = 'container review' id='review_1'>
        <div class='item item1'>★★★★★</div>
        <div class='item item2'>작성자</div>
        <div class='item item3'>작성일자</div>
      </div>
      <div class = 'container review' id='review_2'>
        <div class='item item4'>리뷰 내용...</div>
        <img src="data:image/jpeg;base64," 
        class="img-thumbnail" alt="...">
      </div>
    </div>
    
    <div class = 'container' id = 'qna'>
      <button type="button" class="btn btn-outline-primary">질문 하기</button>
      <button type="button" class="btn btn-outline-primary">내 질문 보기</button>

    <table class="table">
      <thead>
        <tr>
          <th scope="col">답변상태</th>
          <th scope="col">제목</th>
          <th scope="col">작성자</th>
          <th scope="col">작성일</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <th scope="row">처리 중</th>
          <td>질문 3</td>
          <td>작성자 3</td>
          <td>2021.05.01</td>
        </tr>
        <tr>
          <th scope="row">답변 완료</th>
          <td>질문 2</td>
          <td>작성자 2</td>
          <td>2021.04.01</td>
        </tr>
        <tr>
          <th scope="row">답변 완료</th>
          <td >질문 1</td>
          <td>작성자 1</td>
          <td>2021.03.01</td>
        </tr>
      </tbody>
    </table>
    </div>

    <div class='img-container'>
    <img src = 'https://cdn.imweb.me/upload/S2019071811778938fffe1/8d4829e064a29.jpg'>
    </div>


    

    <!-- 제품상세 페이지 입니다. -->
  </div>
</template>

<script>
export default {};
</script>

<style scoped>
#product {
  border: 2px dashed green;
}
.review {
  background: beige;
  height : 100px;
  display: flex;
  /* justify-content: space-around; */
}
#review_1{
  height : 50px;
}

.item{
  width: 100px;
  height: 35px;
  text-align: center;
  margin: 20px;
}
.item4{
  align-self:baseline;
  height: 50px;
  width: 100vh;
  text-align: start;
}

  .img-container {
    text-align: center;
  }
  .img-thumbnail {
    align-self: center;
    height: 100%;
  }
  .review div{
    /* background:darkgrey; */
    border: solid darkgrey;
    
  }
</style>